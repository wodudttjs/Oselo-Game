...
from ai import EgaroucidInspiredAI
...
class OthelloGUI:
    ...
    def setup_game(self):
        ...
        self.ai = EgaroucidInspiredAI(ai_color, difficulty)
    ...
    def draw_board(self):
        ...
        if self.current_player == self.human_color and not self.game_over and not self.ai_thinking:
            for move in self.board.get_valid_moves(self.human_color):
                x, y = move
    ...
    def ai_move(self):
        ...
        def think_and_move():
            start = time.time()
            try:
                print("[AI] Starting move computation...")
                move = self.ai.get_move(self.board)
                elapsed = time.time() - start
                print(f"[AI] Move computed: {move} in {elapsed:.2f} seconds")
                def update_ui():
                    self.ai_thinking = False
                    self.progress.stop()
